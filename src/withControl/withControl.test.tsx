...
const Component: FC<IComponentProps> = ({ focused, pressed, hovered, ...props }) => (
    <div {...props} data-hovered={hovered} data-focused={focused} data-pressed={pressed} />
);
...
describe('ComponentWithControl', () => {
    describe('client environment', () => {
        ...
        test('должен установить focused при фокусе', () => {
            ...
            expect(wrapper.find('div').prop('data-focused')).toBe(undefined);
            expect(
                wrapper
                    .simulate('focus')
                    .update()
        ...
        test('должен установить pressed при нажатии клавиши мыши', () => {
            const wrapper = mount(<ComponentWithControl />);
            expect(wrapper.find('div').prop('data-pressed')).toBe(undefined);
            expect(
                wrapper
                    .simulate('mousedown')
                    .update()
        ...
        test('должен установить hovered при наведении', () => {
            const wrapper = mount(<ComponentWithControl />);
            expect(wrapper.find('div').prop('data-hovered')).toBe(undefined);
            expect(
                wrapper
                    .simulate('mouseenter')
                    .update()
        ...
        test('должен добавлять и удалять атрибут disabled при обновлении через props', () => {
            const wrapper = mount(<ComponentWithControl />);
            expect(wrapper.find('div').prop('disabled')).toBe(undefined);
            wrapper.setProps({ disabled: true });
            expect(wrapper.find('div').prop('disabled')).toBe(true);
            wrapper.setProps({ disabled: false });
            expect(wrapper.find('div').prop('disabled')).toBe(false);
        });
        ...
        test('не должен устанавливать флаги если disabled', () => {
            const wrapper = mount(<ComponentWithControl disabled />);
            wrapper
                .simulate('focus')
                .simulate('mousedown')
                .simulate('mouseenter')
