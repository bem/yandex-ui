...
describe.each<any>(platforms)('Select@%s', (platform, Select: ComponentType<SelectProps>) => {
    ...
    describe('client environment', () => {
        ...
        if (platform === 'desktop') {
            test('при нажатии должен открыться, при потере фокуса - закрыться', () => {
        ...
        test('кнопка мультиселекта с выбранными значениями не получает модификатор `checked` если опция ' +
            '`check` = `false`', () => {
            const wrapper = mount(
                <Select
                    checkable={false}
